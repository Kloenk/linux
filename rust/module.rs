...
fn get_rtnl_links_values(name: &str, netdevice: &str) -> RtnlLinkValues {
    let setup_dev = format!(
        "let mut dev = kernel::net::device::NetDevice::<{}>::from_ptr(dev);",
        netdevice
    );
    match name {
        ...
        "validate" => RtnlLinkValues {
            ...
            wrapper_before: r#"kernel::c_from_kernel_result! {
                let tb = kernel::net::netlink::NlAttrVec::from_ptr(tb as *const *const kernel::bindings::nlattr);
                let data = kernel::net::netlink::NlAttrVec::from_ptr(data as *const *const kernel::bindings::nlattr);
                let extack = kernel::net::netlink::NlExtAck::from_ptr(extack);
                "#.to_string(),
