...
use core::{marker, mem, ops::Deref, ops::DerefMut, ptr};
...
impl<T: NetDeviceAdapter> NetDevice<T> {
    ...
    pub unsafe fn new_locked(
        ...
        let ptr = bindings::alloc_netdev_mqs(
            size,
    ...
    /// hw_addr_random - Generate software assigned random Ethernet and set device flag
    ///
    ...
    /// register - register a network device
    ///
    ...
    pub unsafe fn register_locked(&self) -> KernelResult<()> {
        let err = bindings::register_netdevice(self.ptr as *mut bindings::net_device);
    ...
    /// set_rtnl_ops - set the rtnl_link_ops to a network interface
    ///
    /// Takes a static mut created with [`kernel::net::rtnl_link_ops!`] and assing it to self
    ...
    /// set_rtnl_ops_locked - set the rtnl_link_ops to a network interface, while the caller holds the rtnl_lock
    ///
    /// Takes a static mut created with [`kernel::net::rtnl_link_ops!`] and assing it to self.
    ///
    /// # Safety
    ...
    pub unsafe fn set_rtnl_ops_locked(&self, ops: &'static super::rtnl::RtnlLinkOps) {
        ...
        dev.rtnl_link_ops = ops.get_ptr() as *mut bindings::rtnl_link_ops;
    }
    ...
    pub fn set_new_pcpu_lstats(&mut self) -> KernelResult<()> {
    ...
    pub unsafe fn free_lstats(&mut self) {
        ...
        let net_device: &bindings::net_device = self.deref();
    ...
    /// Return const pointer to the wrapped type.
    pub unsafe fn get_ptr(&self) -> *const bindings::net_device {
        self.ptr
    }
}

impl<I: NetDeviceAdapter> Deref for NetDevice<I> {
    type Target = bindings::net_device;
...
impl<I: NetDeviceAdapter> DerefMut for NetDevice<I> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        // SAFETY: ptr is valid
        unsafe { (self.ptr as *mut bindings::net_device).as_mut() }.unwrap()
    }
}
...
/// maps to [`netdev_tx`] from the kernel.
pub enum NetdevTX {
...
unsafe extern "C" fn ndo_init_callback<T: NetDeviceAdapter>(
    ...
    c_from_kernel_result! {
        T::Ops::init(&mut NetDevice::<T>::from_ptr(dev))?;
        Ok(0)
    }
}
...
unsafe extern "C" fn ndo_uninit_callback<T: NetDeviceAdapter>(dev: *mut bindings::net_device) {
    ...
    //let mut device = dev.as_mut().unwrap();
    T::Ops::uninit(&mut NetDevice::<T>::from_ptr(dev));
}
...
unsafe extern "C" fn ndo_start_xmit_callback<T: NetDeviceAdapter>(
    ...
    //let device = NetDevice::<T>::from_ptr(dev);
...
unsafe extern "C" fn ndo_get_stats64_callback<T: NetDeviceAdapter>(
    ...
    T::Ops::get_stats64(
        ...
        &mut RtnlLinkStats64::from_ptr(stats),
    );
}
...
unsafe extern "C" fn ndo_change_carrier_callback<T: NetDeviceAdapter>(
    ...
    c_from_kernel_result! {
        T::Ops::change_carrier(&mut NetDevice::<T>::from_ptr(dev), change_carrier)?;
        Ok(0)
    }
}
...
unsafe extern "C" fn ndo_validate_addr_callback<T: NetDeviceAdapter>(
    ...
    c_from_kernel_result! {
        T::Ops::validate_addr(&NetDevice::<T>::from_ptr(dev))?;
        Ok(0)
    }
}
...
unsafe extern "C" fn ndo_set_mac_address_callback<T: NetDeviceAdapter>(
    ...
    c_from_kernel_result! {
        T::Ops::set_mac_addr(&mut NetDevice::<T>::from_ptr(dev), p)?;
        Ok(0)
    }
}

unsafe extern "C" fn ndo_set_rx_mode_callback<T: NetDeviceAdapter>(dev: *mut bindings::net_device) {
    T::Ops::set_rx_mode(&mut NetDevice::<T>::from_ptr(dev))
}
...
/// Represents which fields of [`struct net_device_ops`] should pe populated with pointers.
pub struct ToUse {
    /// The `ndo_change_carrier` field of [`struct net_device_ops`].
...
pub trait NetDeviceOps<T: NetDeviceAdapter>: Send + Sync + Sized {
    ...
    fn init(dev: &mut NetDevice<T>) -> KernelResult<()>;
    ...
    fn change_carrier(_dev: &mut NetDevice<T>, _new_carrier: bool) -> KernelResult<()> {
        Err(Error::EINVAL)
    }

    fn validate_addr(_dev: &NetDevice<T>) -> KernelResult<()> {
        Err(Error::EINVAL)
    }

    fn set_mac_addr(_dev: &mut NetDevice<T>, _p: *mut c_types::c_void) -> KernelResult<()> {
        Err(Error::EINVAL)
    }
...
pub mod helpers {
    ...
    pub fn eth_validate_addr<T: NetDeviceAdapter>(dev: &NetDevice<T>) -> KernelResult<()> {
        // SAFETY: dev.ptr is valid if dev is valid
        let ret =
            unsafe { bindings::eth_validate_addr(dev.get_ptr() as *mut bindings::net_device) };
        if ret != 0 {
            Err(Error::from_kernel_errno(ret))
        } else {
            Ok(())
    ...
    pub fn eth_mac_addr<T: NetDeviceAdapter>(
        ...
        // SAFETY: dev.ptr is valid if dev is valid
        let ret = unsafe { bindings::eth_mac_addr(dev.get_ptr() as *mut bindings::net_device, p) };
